...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		{
			//Match Aim.
			aiming = holdingButtonAim && CanAim() && _fpsMode.IsStarted;
			//Match Run.
			running = holdingButtonRun && CanRun() && _fpsMode.IsStarted;

			//Holding the firing button.
			if (holdingButtonFire)
			{
				//Check.
				if (CanPlayAnimationFire() && equippedWeapon.HasAmmunition() && equippedWeapon.IsAutomatic() && _fpsMode.IsStarted)
				{
